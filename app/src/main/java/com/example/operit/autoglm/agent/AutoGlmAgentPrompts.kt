...
object AutoGlmAgentPrompts {
    fun buildUiAutomationSystemPrompt(now: Date = Date()): String {
        val sdf = SimpleDateFormat("yyyy年MM月dd日 EEEE", Locale.CHINA)
        val currentDate = sdf.format(now)
        return UI_AUTOMATION_AGENT_PROMPT.replace("{{current_date}}", currentDate)
    }

    /**
     * 迁移自 Operit：FunctionalPrompts.UI_AUTOMATION_AGENT_PROMPT
     *
     * autoglm-phone 风格：模型输出 <think> 与 <answer>，answer 中是 do(...) 或 finish(...)
     */
...
- do(action="Long Press", element=[x,y])
    Long Press是长按操作，在屏幕上的特定点长按指定时间。可用于触发上下文菜单、选择文本或激活长按交互。坐标系统从左上角 (0,0) 开始到右下角（999,999)结束。此操作完成后，您将自动收到结果状态的截图。
- do(action="Double Tap", element=[x,y])
    Double Tap是双击操作，在屏幕上的特定点连续点击两次。此操作可以激活双击交互，如缩放、选择文本或打开项目。坐标系统从左上角 (0,0) 开始到右下角（999,999)结束。此操作完成后，您将自动收到结果状态的截图。
