...
object ShowerServerManager {
    ...
    suspend fun ensureServerStarted(context: Context): Boolean {
        ...
        // 3) Copy the jar from app external files dir to /data/local/tmp using shell identity,
        // so that the resulting file is owned by the shell user.
        val copyCmd = "cp ${escapeShellArg(jarFile.absolutePath)} ${escapeShellArg(remoteJarPath)}"
        ...
        val startCmd = "CLASSPATH=${escapeShellArg(remoteJarPath)} app_process / com.ai.assistance.shower.Main &"
    ...
    private suspend fun copyJarToExternalDir(context: Context): File = withContext(Dispatchers.IO) {
        // 使用 App 专属 external files 目录，避免 Android 10+ 的分区存储限制。
        // 通常路径：/sdcard/Android/data/<package>/files/Operit/shower-server.jar
        val root =
            context.getExternalFilesDir(null)
                ?: throw IllegalStateException("External files dir unavailable")
        val baseDir = File(root, "Operit")
        if (!baseDir.exists() && !baseDir.mkdirs()) {
            throw IllegalStateException("Failed to create dir: ${baseDir.absolutePath}")
        }
